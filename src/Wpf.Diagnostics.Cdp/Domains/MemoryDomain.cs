    ...
    public static async Task<JsonObject> HandleAsync(CdpSession session, string action, JsonObject @params)
    {
        switch (action)
        {
            case "getDOMCounters":
                {
                    ...
                    {
                        nodes = await session.Window.Dispatcher.InvokeAsync(() => CountVisuals(session.Window));
                    }
            ...
            case "getLiveControls":
                {
                    if (session.Window == null) return new JsonObject { ["controls"] = new JsonArray() };
            ...
            case "getDetachedControls":
                {
                    ...
                    var detachedList = await session.Window.Dispatcher.InvokeAsync(() =>
                    {
            ...
            case "takeHeapSnapshot":
                {
                    ...
                    var snapshotJsonObj = await session.Window.Dispatcher.InvokeAsync(() =>
                    {
                        var visitedVisuals = new HashSet<Visual>();
                        var orderedVisuals = new List<Visual>();

                        void Traverse(Visual visualObj)
                        {
                            ...
                            {
                                if (VisualTreeHelper.GetChild(visualObj, i) is Visual child)
                                {
                                    Traverse(child);
                                }
                            }
                        ...
                        foreach (var visual in orderedVisuals)
                        {
                            ...
                            for (int i = 0; i < count; i++)
                            {
                                if (VisualTreeHelper.GetChild(visual, i) is Visual child && nodesMap.TryGetValue(child, out var childNode))
                                {
                                    visualNode.Edges.Add(new HeapEdge
                                    {
                                        TypeIndex = 1, // element
            ...
            case "getRetainers":
                {
                    ...
                    var tree = await session.Window.Dispatcher.InvokeAsync(() =>
                    {
