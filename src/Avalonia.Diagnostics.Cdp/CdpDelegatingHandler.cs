...
public class CdpDelegatingHandler : DelegatingHandler
{
    ...
    {
        // 1. Offline Check
        if (Avalonia.Diagnostics.Cdp.Domains.NetworkDomain.Offline)
        {
            throw new HttpRequestException("Network is offline (emulated by CDP).");
        }

        // 2. Latency Delay
        if (Avalonia.Diagnostics.Cdp.Domains.NetworkDomain.Latency > 0)
        {
            await Task.Delay((int)Avalonia.Diagnostics.Cdp.Domains.NetworkDomain.Latency, cancellationToken).ConfigureAwait(false);
        }

        // 3. URL Blocking Check
        var requestUrl = request.RequestUri?.ToString() ?? "";
        if (Avalonia.Diagnostics.Cdp.Domains.NetworkDomain.IsBlocked(requestUrl))
        {
