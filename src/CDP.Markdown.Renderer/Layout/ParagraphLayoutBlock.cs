...
public class ParagraphLayoutBlock : ILayoutBlock
{
    public ParagraphBlock Node { get; }
    ...
    public void Layout(LayoutContext context)
    {
        ...
        // Add paragraph spacing margin bottom (10px)
        blockHeight += 10.0f;
                ...
                foreach (var run in line.Runs)
                {
                    ...
                    else if (run.IsImage)
                    {
                        var imgRect = new SKRect(
                            ...
                            run.LocalBounds.Bottom + line.Bounds.Top - 10f
                        );
                        var bitmap = MarkdownImageLoader.GetOrLoadImage(run.ImageUrl, context.OnImageLoaded);
                        if (bitmap != null)
                        {
                        ...
                        {
                            canvas.DrawRect(imgRect, context.Resources.ImagePlaceholderBorderPaint);
                            
                            float iconSize = 24f;
                            float iconX = imgRect.Left + (imgRect.Width - iconSize) / 2f;
                            float iconY = imgRect.Top + (imgRect.Height - iconSize) / 2f - 10f;
                            canvas.DrawRect(new SKRect(iconX, iconY, iconX + iconSize, iconY + iconSize), context.Resources.BulletPaint);
                            
                            if (!string.IsNullOrEmpty(run.AltText))
                            {
                                float altTextX = imgRect.Left + 10f;
                                float altTextY = imgRect.Bottom - 15f;
                                canvas.DrawText(run.AltText, altTextX, altTextY, context.Resources.TextFont, context.Resources.TextPaint);
                            }
                        }
                    }
                    else
                    {
                        float baseline = context.Resources.TextFont.Spacing * 0.75f;
                        ...
                        {
                            canvas.DrawText(run.Text, run.LocalBounds.Left + line.Bounds.Left, run.LocalBounds.Top + line.Bounds.Top + baseline, context.Resources.TextFont, run.Paint);
                        }
                        ...
                        {
                            float lineY = run.LocalBounds.Top + line.Bounds.Top + baseline - (context.Resources.TextFont.Size * 0.3f);
                            canvas.DrawLine(
                                run.LocalBounds.Left + line.Bounds.Left,
                                lineY,
                                run.LocalBounds.Right + line.Bounds.Left,
