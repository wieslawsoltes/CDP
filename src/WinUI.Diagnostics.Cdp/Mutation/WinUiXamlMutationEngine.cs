...
public class WinUiXamlMutationEngine : IMutationEngine
{
    private static readonly ILogger Logger = CdpLogging.CreateLogger<WinUiXamlMutationEngine>();
    private readonly CdpSession _session;
    private readonly object _scanLock = new();
    private bool _scanned = false;
    private readonly ConcurrentDictionary<string, string> _classToFileMap = new(StringComparer.Ordinal);

    public WinUiXamlMutationEngine(CdpSession session)
    {
        _session = session;
    }
    ...
    public async Task<bool> SetAttributeAsync(object target, string name, string value)
    {
        ...
        {
            await _session.Window.DispatcherQueue.InvokeAsync(() =>
            {
