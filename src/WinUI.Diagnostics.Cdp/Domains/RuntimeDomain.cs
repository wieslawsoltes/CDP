...
public class CdpRuntimeDocument
{
    ...
    public CdpRuntimeElement? documentElement => _session.Window?.Content is UIElement root ? new CdpRuntimeElement(root, _session) : null;
    public CdpRuntimeElement? body => documentElement;
    
    public CdpRuntimeElement? querySelector(string selector)
    {
        if (_session.Window?.Content == null) return null;
        var match = SelectorEngine.QuerySelector(_session.Window.Content, selector, _session.UseLogicalTree);
        return match != null ? new CdpRuntimeElement(match, _session) : null;
    }

    public IEnumerable<CdpRuntimeElement> querySelectorAll(string selector)
    {
        if (_session.Window?.Content == null) return Enumerable.Empty<CdpRuntimeElement>();
        var matches = SelectorEngine.QuerySelectorAll(_session.Window.Content, selector, _session.UseLogicalTree);
        return matches.Select(m => new CdpRuntimeElement(m, _session));
    }
...
public class CdpRuntimeWindow
{
    ...
    public Window? window => _session.Window;
    public double innerWidth => window?.Bounds.Width ?? 0;
    public double innerHeight => window?.Bounds.Height ?? 0;
    public double devicePixelRatio => 1.0;
    public string title => _session.Target?.Title ?? "WinUI/Uno Application";
...
public class CdpRuntimeElement
{
    ...
    public CdpRuntimeElement(UIElement element, CdpSession session)
    {
        visual = element;
        _session = session;
    }
    ...
    public double clientWidth => visual is FrameworkElement fe ? fe.ActualWidth : 0;
    public double clientHeight => visual is FrameworkElement fe ? fe.ActualHeight : 0;
    ...
    public CdpClientRect getBoundingClientRect()
    {
        ...
        if (_session.Window?.Content != null)
        {
            try
            {
                var transform = visual.TransformToVisual(_session.Window.Content);
...
public class WinUiScriptGlobals
{
    private readonly CdpSession _session;

    public WinUiScriptGlobals(CdpSession session)
    {
        _session = session;
    }

    public UIElement? SelectedNode => _session.NodeMap.GetVisual(_session.InspectedNodeId) as UIElement;
    ...
    {
        var root = Window?.Content as UIElement ?? SelectedNode;
        return root != null ? SelectorEngine.QuerySelector(root, selector, _session.UseLogicalTree) : null;
    }

    public IEnumerable<UIElement> QueryAll(string selector)
    {
        var root = Window?.Content as UIElement ?? SelectedNode;
        return root != null ? SelectorEngine.QuerySelectorAll(root, selector, _session.UseLogicalTree) : Enumerable.Empty<UIElement>();
    }
}
...
public static class RuntimeDomain
{
            ...
            case "getProperties":
                {
                    ...
                    if (session.Window == null) return new JsonObject { ["result"] = new JsonArray() };
    ...
    private static async Task<JsonObject> EvaluateJsAsync(CdpSession session, string expression, bool returnByValue)
    {
        ...
        return await dispatcher.InvokeAsync(() =>
        {
            try
            {
                var engine = _engines.GetOrAdd(session.GetHashCode().ToString(), s =>
                {
                    var eng = new Engine();
                ...
                var selectedVisual = session.NodeMap.GetVisual(session.InspectedNodeId) as UIElement;
                engine.SetValue("$0", selectedVisual != null ? new CdpRuntimeElement(selectedVisual, session) : null);
    ...
    private static async Task<JsonObject> EvaluateCSharpAsync(CdpSession session, string expression, bool returnByValue)
    {
        ...
        return await dispatcher.InvokeAsync(async () =>
        {
            try
            {
                var globals = new WinUiScriptGlobals(session);
                var scriptOptions = ScriptOptions.Default
                    .AddImports("System", "System.Collections.Generic", "System.Linq", "Microsoft.UI.Xaml", "Microsoft.UI.Xaml.Controls", "Microsoft.UI.Xaml.Media")
                    .AddReferences(
