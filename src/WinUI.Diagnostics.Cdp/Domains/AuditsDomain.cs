...
public static class AuditsDomain
{
            ...
            case "runDiagnostics":
                {
                    ...
                    return await session.Window.DispatcherQueue.InvokeAsync(() =>
                    {
                        ...
                        RunDiagnosticAudits(session.Window.Content, 0, issues, ref a11yScore, ref bestPracticesScore, ref layoutScore, session.NodeMap);
    ...
    private static Windows.UI.Color ResolveCompositeBackground(UIElement startVisual)
    {
        ...
        if (aAcc > 0)
        {
            ...
            return Windows.UI.Color.FromArgb((byte)Math.Clamp(aAcc * 255.0, 0, 255), (byte)Math.Clamp(rAcc, 0, 255), (byte)Math.Clamp(gAcc, 0, 255), (byte)Math.Clamp(bAcc, 0, 255));
        }
        ...
        if (visual is TextBlock textBlock && !string.IsNullOrWhiteSpace(textBlock.Text))
        {
            if (textBlock.Foreground is SolidColorBrush solidFore)
            {
                ...
                if (foreColor.A < 255)
                {
                    ...
                    foreColor = Windows.UI.Color.FromArgb(255, (byte)Math.Clamp(rComp, 0, 255), (byte)Math.Clamp(gComp, 0, 255), (byte)Math.Clamp(bComp, 0, 255));
                }
                ...
                double minRatio = 4.5;
                bool isBold = textBlock.FontWeight.Weight >= 700;
