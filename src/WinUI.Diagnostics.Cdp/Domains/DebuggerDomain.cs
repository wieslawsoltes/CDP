    ...
    public static void CheckBreakpoint(CdpSession session, string url, int line)
    {
        ...
        if (hit && hitBp != null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(hitBp.Condition))
            {
                ...
                if (session.Window != null && session.Window.DispatcherQueue.HasThreadAccess)
                {
                    shouldPause = evalCondition();
                }
                else if (session.Window != null)
                {
                    shouldPause = session.Window.DispatcherQueue.InvokeAsync(evalCondition).GetAwaiter().GetResult();
                }
            }
