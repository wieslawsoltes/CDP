...
public static class DomDomain
{
            ...
            case "describeNode":
                {
                    ...
                    else if (!string.IsNullOrEmpty(objectId))
                    {
                        ...
                        {
                            targetVisual = session.Window?.Content as UIElement;
                        }
                        else
                        {
            ...
            case "resolveNode":
                {
                    ...
                    var visual = session.NodeMap.GetVisual(resolvedNodeId) as UIElement;
                    if (visual == null)
                    {
            ...
            case "focus":
                {
                    ...
                    if (visual is UIElement ui)
                    {
                        ...
                        {
                            if (ui is Control ctrl) ctrl.Focus(FocusState.Programmatic);
                        });
                    }
                    return new JsonObject();
                }
            ...
            case "setAttributeValue":
                {
                    ...
                    if (visual != null)
                    {
                        ...
                        {
                            var adapter = new WinUiFrameworkAdapter(session.Window.DispatcherQueue, session.NodeMap);
    ...
    private static JsonObject BuildDocumentNode(CdpSession session, int maxDepth)
    {
        var rootNode = new JsonObject
        {
            ...
            ["localName"] = "",
            ["nodeValue"] = "",
            ["childNodeCount"] = session.Window?.Content != null ? 1 : 0,
            ["documentURL"] = $"http://localhost:{CdpServer.Port}/",
            ["baseURL"] = $"http://localhost:{CdpServer.Port}/",
            ["xmlVersion"] = ""
        };

        if (session.Window?.Content is UIElement rootElement)
        {
            var children = new JsonArray();
            children.Add(BuildDomNode(rootElement, session, 1, maxDepth));
    ...
    public static JsonObject BuildDomNode(UIElement visual, CdpSession session, int currentDepth, int maxDepth)
    {
    ...
    public static string GetOuterHtml(UIElement visual, CdpSession session)
    {
        ...
        if (visual is FrameworkElement ctrl)
        {
            ...
            {
                sb.Append(" text=\"").Append(System.Net.WebUtility.HtmlEncode(text)).Append("\"");
            }
        }
        ...
        else
        {
            ...
            {
                sb.Append(">").Append(System.Net.WebUtility.HtmlEncode(txtContent)).Append("</").Append(name).Append(">");
            }
            else
            {
                sb.Append(" />");
    ...
    private static string GetLocalName(UIElement visual)
    {
        return visual.GetType().Name.ToLowerInvariant();
    }

    private static string GetMappedTagName(UIElement visual)
    {
    ...
    private static IEnumerable<UIElement> GetChildren(UIElement visual, CdpSession session)
    {
        return CdpVisualTreeHelper.GetChildren(visual, session.UseLogicalTree);
    }

    private static JsonArray BuildAttributes(DependencyObject visual)
    {
    ...
    private static JsonObject BuildBoxModel(UIElement visual, CdpSession session)
    {
        double width = 0;
        double height = 0;
        ...
        if (session.Window?.Content != null)
        {
            try
            {
                var transform = visual.TransformToVisual(session.Window.Content);
