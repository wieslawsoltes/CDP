...
public static class DomDebuggerDomain
{
            ...
            case "getEventListeners":
                {
                    ...
                    {
                        listeners = await session.Window.DispatcherQueue.InvokeAsync(() => GetListeners(session, target));
                    }
    ...
    private static string MapEventName(string winUiName)
    {
        return winUiName switch
        {
            "Click" => "click",
