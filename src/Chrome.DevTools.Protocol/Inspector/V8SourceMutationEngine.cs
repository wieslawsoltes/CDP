...
public sealed class V8SourceMutationEngine
{
        ...
        string generatedSource)
    {
        ...
        {
            return V8SourceMutationResult.NoChange(originalSource, generatedSource, sourceMap);
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var regenerator in _regenerators)
        {
            ...
            {
                diagnostics.Add($"{regenerator.Name}: {ex.Message}");
                continue;
            }

            if (!regenerated.Success)
            {
                diagnostics.Add($"{regenerator.Name}: {regenerated.Message}");
                continue;
            }
            if (regenerated.SourceMap is null)
            {
                diagnostics.Add($"{regenerator.Name}: regeneration did not return a source map.");
                continue;
            }
            if (sourceIndex >= regenerated.SourceMap.Sources.Count)
            {
                diagnostics.Add($"{regenerator.Name}: regeneration returned an incompatible source map.");
                continue;
            }
            ...
            {
                diagnostics.Add($"{regenerator.Name}: source map does not contain the edited source.");
                continue;
            }
        ...
        if (compatibleAdapters > 0)
        {
            return V8SourceMutationResult.Rejected(
                $"All compatible compiler adapters failed. {string.Join(" | ", diagnostics)}");
        }
        return V8SourceMutationResult.Rejected(
...
{
    public V8SourceMutationPreview? Preview { get; init; }
    ...
    internal static V8SourceMutationResult Ready(
        ...
        string originalSource,
        string editedSource,
        string previousGeneratedSource) =>
        ...
        {
            Preview = V8SourceMutationPreview.Create(V8SourceMutationKind.MappedPatch, "", originalSource,
                editedSource, previousGeneratedSource, generatedSource)
        };
    ...
    internal static V8SourceMutationResult Regenerated(
        ...
        {
            Preview = V8SourceMutationPreview.Create(V8SourceMutationKind.Regenerated, adapterName, originalSource,
                editedSource, previousGeneratedSource, generatedSource)
        };

    internal static V8SourceMutationResult NoChange(string originalSource, string generatedSource, V8SourceMap sourceMap) =>
        new(true, false, "The source is unchanged.", generatedSource, sourceMap, null, null, V8SourceMutationKind.None)
        {
            Preview = V8SourceMutationPreview.Create(V8SourceMutationKind.None, "", originalSource, originalSource,
