...
test.describe('CDP Recorded Tests', () => {
  test('recorded test', async () => {
    ...
    await test.step('Assert True: !string.IsNullOrEmpty(((CdpInspectorApp.ViewModels.MainWindowViewModel)Window.DataContext).Recorder.TestStudio.LastReportPath)', async () => {
      const result = await page.evaluate('!string.IsNullOrEmpty(((CdpInspectorApp.ViewModels.MainWindowViewModel)Window.DataContext).Recorder.TestStudio.LastReportPath)');
      await expect(result).toBeTruthy();
    });

    await test.step('Assert True: System.IO.File.Exists(((CdpInspectorApp.ViewModels.MainWindowViewModel)Window.DataContext).Recorder.TestStudio.LastReportPath)', async () => {
      const result = await page.evaluate('System.IO.File.Exists(((CdpInspectorApp.ViewModels.MainWindowViewModel)Window.DataContext).Recorder.TestStudio.LastReportPath)');
      await expect(result).toBeTruthy();
    });

    await test.step('Tap on element #btnViewHtmlReport', async () => {
      const element_22 = page.locator('#btnViewHtmlReport');
