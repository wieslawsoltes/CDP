...
test.describe('CDP Recorded Tests', () => {
  test('recorded test', async () => {
    ...
    await test.step('Evaluate Script: __raw_window.DataContext.Connection.ResetForm()', async () => {
      await page.evaluate('__raw_window.DataContext.Connection.ResetForm()');
    });

    await test.step('Type text in element #txtHost', async () => {
      const element_7 = page.locator('#txtHost');
      await element_7.fill('192.168.1.100');
    });

    await test.step('Assert True: __raw_window.DataContext.Connection.Host == \'192.168.1.100\'', async () => {
      const result = await page.evaluate('__raw_window.DataContext.Connection.Host == \'192.168.1.100\'');
      await expect(result).toBeTruthy();
    });
