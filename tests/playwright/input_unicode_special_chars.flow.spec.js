...
test.describe('CDP Recorded Tests', () => {
  test('recorded test', async () => {
    ...
    await test.step('Type text in element #txtHost', async () => {
      ...
      await element_6.fill('test_🚀_c#_ñ_€');
    });

    await test.step('Assert True: __raw_window.DataContext.Connection.Host.Contains(\'🚀\')', async () => {
      const result = await page.evaluate('__raw_window.DataContext.Connection.Host.Contains(\'🚀\')');
      await expect(result).toBeTruthy();
    });
