...
test.describe('CDP Recorded Tests', () => {
  test('recorded test', async () => {
    ...
    await test.step('Evaluate Script: __raw_window.DataContext.Elements.SelectedNodePath', async () => {
      await page.evaluate('__raw_window.DataContext.Elements.SelectedNodePath');
    });

    await test.step('Assert True: document.querySelector(\'#treeRoot\') != null', async () => {
      const result = await page.evaluate('document.querySelector(\'#treeRoot\') != null');
      await expect(result).toBeTruthy();
    });
