...
test.describe('CDP Recorded Tests', () => {
  test('recorded test', async () => {
    ...
    await test.step('Press key unknown_key_999', async () => {
      await page.keyboard.press('unknown_key_999');
    });
    ...
    await test.step('Assert True: __raw_window.DataContext.HasInputError == false', async () => {
      const result = await page.evaluate('__raw_window.DataContext.HasInputError == false');
      await expect(result).toBeTruthy();
    });
