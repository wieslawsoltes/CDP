...
test.describe('CDP Recorded Tests', () => {
  test('recorded test', async () => {
    ...
    await test.step('Double tap on element #lstRemoteItems', async () => {
      const element_10 = page.locator('#lstRemoteItems');
      await element_10.dblclick();
    });
    ...
    await test.step('Assert True: __raw_window.DataContext.ItemSelected == true', async () => {
      const result = await page.evaluate('__raw_window.DataContext.ItemSelected == true');
      await expect(result).toBeTruthy();
    });
