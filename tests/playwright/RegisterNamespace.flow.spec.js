...
test.describe('CDP Recorded Tests', () => {
  test('recorded test', async () => {
    ...
    await test.step('Tap on element #treeWorkspaceFiles', async () => {
      const element_5 = page.locator('#treeWorkspaceFiles');
      await element_5.tap();
    });
    ...
    await test.step('Assert True: ((CdpInspectorApp.ViewModels.MainWindowViewModel)Window.DataContext).Connection.IsConnected', async () => {
      const result = await page.evaluate('((CdpInspectorApp.ViewModels.MainWindowViewModel)Window.DataContext).Connection.IsConnected');
      await expect(result).toBeTruthy();
    });
