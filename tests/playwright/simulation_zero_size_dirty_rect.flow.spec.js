...
test.describe('CDP Recorded Tests', () => {
  test('recorded test', async () => {
    ...
    await test.step('Evaluate Script: __raw_window.DataContext.Preview.ApplyDirtyRect(0, 0, 0, 0)', async () => {
      await page.evaluate('__raw_window.DataContext.Preview.ApplyDirtyRect(0, 0, 0, 0)');
    });
    ...
    await test.step('Assert True: __raw_window.DataContext.Preview.HasError == false', async () => {
      const result = await page.evaluate('__raw_window.DataContext.Preview.HasError == false');
      await expect(result).toBeTruthy();
    });
