...
test.describe('CDP Recorded Tests', () => {
  test('recorded test', async () => {
    ...
    await test.step('Clear text in element #txtHost', async () => {
      ...
      await element_6.clear();
    });

    await test.step('Type text in element #txtHost', async () => {
      const element_7 = page.locator('#txtHost');
      await element_7.fill('');
    });

    await test.step('Assert True: __raw_window.DataContext.Connection.Host == \'\'', async () => {
      const result = await page.evaluate('__raw_window.DataContext.Connection.Host == \'\'');
      await expect(result).toBeTruthy();
    });
