...
test.describe('CDP Recorded Tests', () => {
  test('recorded test', async () => {
    ...
    await test.step('Tap on element #btnTestStudioToggleRecord', async () => {
      const element_8 = page.locator('#btnTestStudioToggleRecord');
      await element_8.tap();
    });
    ...
    await test.step('Assert True: __raw_window.DataContext.Recorder.FrameCount > 5', async () => {
      const result = await page.evaluate('__raw_window.DataContext.Recorder.FrameCount > 5');
      await expect(result).toBeTruthy();
    });

    await test.step('Assert True: __raw_window.DataContext.SampleButtonClickCount == 10', async () => {
      const result = await page.evaluate('__raw_window.DataContext.SampleButtonClickCount == 10');
      await expect(result).toBeTruthy();
    });
