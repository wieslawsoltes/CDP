...
test.describe('CDP Recorded Tests', () => {
  test('recorded test', async () => {
    ...
    await test.step('Evaluate Script: __raw_window.DataContext.Server.SimulateSecondaryClientConnection()', async () => {
      await page.evaluate('__raw_window.DataContext.Server.SimulateSecondaryClientConnection()');
    });
    ...
    await test.step('Assert True: __raw_window.DataContext.Server.ActiveSessionCount >= 1', async () => {
      const result = await page.evaluate('__raw_window.DataContext.Server.ActiveSessionCount >= 1');
      await expect(result).toBeTruthy();
    });
