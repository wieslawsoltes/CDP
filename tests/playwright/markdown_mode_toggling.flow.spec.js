...
test.describe('CDP Recorded Tests', () => {
  test('recorded test', async () => {
    ...
    await test.step('Assert element #txtSourceContent is hidden', async () => {
      await expect(page.locator('#txtSourceContent')).toBeHidden();
    });

    await test.step('Tap on element #btnToggleMarkdownMode', async () => {
      const element_19 = page.locator('#btnToggleMarkdownMode');
    ...
    await test.step('Assert element #mdVisualEditor is hidden', async () => {
      await expect(page.locator('#mdVisualEditor')).toBeHidden();
    });

    await test.step('Tap on element #btnToggleMarkdownMode', async () => {
      const element_23 = page.locator('#btnToggleMarkdownMode');
      await element_23.tap();
    });
