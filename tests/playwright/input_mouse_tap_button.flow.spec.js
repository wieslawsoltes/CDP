...
test.describe('CDP Recorded Tests', () => {
  test('recorded test', async () => {
    ...
    await test.step('Tap on element #btnSampleAction', async () => {
      const element_10 = page.locator('#btnSampleAction');
    ...
    await test.step('Assert True: __raw_window.DataContext.SampleButtonClicked == true', async () => {
      const result = await page.evaluate('__raw_window.DataContext.SampleButtonClicked == true');
      await expect(result).toBeTruthy();
    });
