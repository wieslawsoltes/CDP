...
test.describe('CDP Recorded Tests', () => {
  test('recorded test', async () => {
    ...
    await test.step('Tap on element #btnDisconnect', async () => {
      const element_7 = page.locator('#btnDisconnect');
      await element_7.tap();
    });
    ...
    await test.step('Assert True: __raw_window.DataContext.Connection.StatusText == \'Disconnected\'', async () => {
      const result = await page.evaluate('__raw_window.DataContext.Connection.StatusText == \'Disconnected\'');
      await expect(result).toBeTruthy();
    });
