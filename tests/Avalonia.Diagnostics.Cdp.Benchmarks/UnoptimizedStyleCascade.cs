...
namespace UnoptimizedCDP.Html.Renderer.Style;

public static class UnoptimizedStyleCascade
{
        ...
        Dictionary<HtmlNode, ComputedStyle> resolved)
    {
        ...
        if (node is HtmlElement element)
        {
            ...
            var matchedRules = new List<MatchingRuleInfo>();
            foreach (var rule in stylesheet.Rules)
            {
                foreach (var selector in rule.Selectors)
                {
                    ...
                    {
                        matchedRules.Add(new MatchingRuleInfo
                        {
                            Rule = rule,
                            Selector = selector,
                            Index = matchedRules.Count
                        });
                    }
                }
            }
            ...
            {
                var sortedRules = matchedRules.OrderBy(x => x, MatchingRuleInfoComparer.Instance).ToList();
                foreach (var ruleInfo in sortedRules)
                {
