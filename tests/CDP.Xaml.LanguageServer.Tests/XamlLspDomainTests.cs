...
namespace Avalonia.Diagnostics.Cdp.Tests
{
    ...
    <Button Grid.R />
        ...
        [Fact]
        public async Task TestCompletions_LocalClrNamespaceWithoutAssembly_ResolvesAndReturnsCompletions()
        {
            string repoRoot = FindRepoRoot();
            string tempFile = Path.Combine(repoRoot, "TempClrNamespaceNoAssembly.axaml");
            
            string xaml = @"<Window xmlns=""https://github.com/avaloniaui""
        xmlns:x=""http://schemas.microsoft.com/winfx/2006/xaml""
        xmlns:hl=""clr-namespace:Avalonia.Diagnostics.Cdp"">
    <hl:HighlightAdorner >
    </hl:HighlightAdorner>
</Window>";
            await File.WriteAllTextAsync(tempFile, xaml);
        ...
        [InlineData("clr-namespace:;assembly=")]
        [InlineData("clr-namespace:MyNamespace;assembly=")]
        [InlineData("clr-namespace:MyNamespace;assembly=NonExistent")]
        [InlineData("clr-namespace:NonExistentNamespace")]
        [InlineData("clr-namespace:;")]
        [InlineData("clr-namespace::")]
        [InlineData("invalid-schema-uri")]
        [InlineData("")]
        [InlineData(":")]
        public async Task TestCompletions_EdgeCaseNamespaces_DoesNotCrash(string ns)
        {
            string repoRoot = FindRepoRoot();
            string tempFile = Path.Combine(repoRoot, $"TempEdgeCase_{Guid.NewGuid():N}.axaml");
    ...
    <hl:SomeControl />
        ...
        [InlineData("")]
        [InlineData(":")]
        public async Task TestHover_EdgeCaseNamespaces_DoesNotCrash(string ns)
        {
            string repoRoot = FindRepoRoot();
            string tempFile = Path.Combine(repoRoot, $"TempEdgeCase_{Guid.NewGuid():N}.axaml");
    ...
    <hl:SomeControl />
        ...
        [Fact]
        public void TestSchemaRegistry_EnsureNamespaceRegistered_DuplicateRegistration()
        {
            var registry = new Xaml.Compiler.Registry.XamlSchemaRegistry();
            string nsUri = "clr-namespace:Avalonia.Diagnostics.Cdp";

            registry.EnsureNamespaceRegistered(nsUri);
            registry.EnsureNamespaceRegistered(nsUri);

            var list = registry.Namespaces.Where(n => n.Uri == nsUri).ToList();
            Assert.Single(list);
            Assert.Equal(nsUri, list[0].Uri);
        }
        ...
        [InlineData("")]
        [InlineData(null)]
        public void TestSchemaRegistry_EnsureNamespaceRegistered_EdgeCases(string ns)
        {
            var registry = new Xaml.Compiler.Registry.XamlSchemaRegistry();
            registry.EnsureNamespaceRegistered(ns);

            if (ns != null && ns.StartsWith("clr-namespace:", StringComparison.OrdinalIgnoreCase))
            {
                var list = registry.Namespaces.Where(n => n.Uri == ns).ToList();
                Assert.Single(list);
            }
            else
            {
        ...
        [Fact]
        public async Task TestSchemaRegistry_EnsureNamespaceRegistered_ThreadConcurrency()
        {
            var registry = new Xaml.Compiler.Registry.XamlSchemaRegistry();
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                tasks.Add(Task.Run(() =>
                {
                    // Mix of duplicate and unique namespaces
                    registry.EnsureNamespaceRegistered("clr-namespace:Avalonia.Diagnostics.Cdp");
                    registry.EnsureNamespaceRegistered($"clr-namespace:UniqueNamespace_{index % 10}");
                }));
            }

            await Task.WhenAll(tasks);

            // Expect 1 (Cdp namespace) + 10 (UniqueNamespace_0..9) = 11 namespaces starting with clr-namespace
            var count = registry.Namespaces.Count(n => n.Uri.StartsWith("clr-namespace:", StringComparison.OrdinalIgnoreCase));
            Assert.Equal(11, count);
        }

        [Fact]
        public async Task TestXamlLspDomain_ConcurrencyCompletions()
        {
            string repoRoot = FindRepoRoot();
            string tempFile = Path.Combine(repoRoot, $"TempConcurrency_{Guid.NewGuid():N}.axaml");
            ...
            try
            {
                ...
                for (int i = 0; i < 50; i++)
                {
                    tasks.Add(Task.Run(async () =>
                    {
                        ...
                        return await XamlLspDomain.HandleAsync(null!, "getCompletions", @params);
                    }));
                }
