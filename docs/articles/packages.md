...
This guide details all **14 NuGet packages** that make up this ecosystem.
...
| **Chrome.DevTools.Automation.Headless** | Headless E2E automation support and test helpers | Preview / Centralized |
...
| Property | Description |
| :--- | :--- |
| **Package ID** | `Chrome.DevTools.Automation.Appium` |
| **Install Command** | `dotnet add package Chrome.DevTools.Automation.Appium --prerelease` |
| **When to Use** | Use when creating C# Appium E2E testing suites to automatically handle target processes and driver instances. |
| **Project References** | None |
| **NuGet Dependencies** | `Appium.WebDriver`, `Selenium.WebDriver`, `xunit.v3.extensibility.core` |

#### Key Types
* `CdpAppiumOrchestrator.cs`: Starts and monitors target applications and background Node.js proxy driver processes.
* `CdpAppiumFixture.cs`: xUnit test class lifecycle fixture mapping initialization and disposal handlers.
* `CdpAppiumOptions.cs`: Configurable parameters governing ports, execution paths, and arguments.

#### Usage Example
```csharp
using CDP.Automation.Appium;

public class MyTests : IClassFixture<CdpAppiumFixture>
{
    // CdpAppiumFixture automatically launches CdpSampleApp and custom proxy
}
```
...
| Property | Description |
| :--- | :--- |
| **Package ID** | `Chrome.DevTools.Automation.Selenium` |
| **Install Command** | `dotnet add package Chrome.DevTools.Automation.Selenium --prerelease` |
| **When to Use** | Use when creating C# Selenium E2E testing suites to automatically handle target processes and driver instances. |
| **Project References** | None |
| **NuGet Dependencies** | `Selenium.WebDriver`, `xunit.v3.extensibility.core` |

#### Key Types
* `CdpSeleniumOrchestrator.cs`: Starts and monitors target applications and launches ChromeDriver.
* `CdpSeleniumFixture.cs`: xUnit test class lifecycle fixture mapping initialization and disposal handlers.
* `CdpSeleniumOptions.cs`: Configurable parameters governing ports, execution paths, and arguments.

#### Usage Example
```csharp
using CDP.Automation.Selenium;

public class MyTests : IClassFixture<CdpSeleniumFixture>
{
    // CdpSeleniumFixture automatically launches CdpSampleApp and ChromeDriver
}
```
...
| Property | Description |
| :--- | :--- |
| **Package ID** | `Chrome.DevTools.Automation.Headless` |
| **Install Command** | `dotnet add package Chrome.DevTools.Automation.Headless --prerelease` |
| **When to Use** | Use when writing in-process headless Avalonia tests (using `Avalonia.Headless` or `Avalonia.Headless.XUnit`) to reuse click, type, and drag-and-drop actions. |
| **Project References** | None |
| **NuGet Dependencies** | `Avalonia`, `Avalonia.Headless` |

#### Key Types
* `CdpHeadlessHelper.cs`: Provides `window.ClickControl(...)` and `window.DragAndDrop(...)` extension methods on Avalonia windows.

#### Usage Example
```csharp
using CDP.Automation.Headless;

// Simulated click on element
window.ClickControl(element, MouseButton.Left, RawInputModifiers.None);
```
