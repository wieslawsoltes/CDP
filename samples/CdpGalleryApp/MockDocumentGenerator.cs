...
public static class MockDocumentGenerator
{
    public static string GenerateDocx()
    {
        string path = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString() + ".docx");
        using (var fs = new FileStream(path, FileMode.Create, FileAccess.ReadWrite))
        using (var wordDoc = WordprocessingDocument.Create(fs, WordprocessingDocumentType.Document, true))
        {
            ...
            p.AppendChild(new W.Run(new W.Text("New Line - Rich DOCX Content")));
            body.AppendChild(p);
    ...
    public static string GenerateXlsx()
    {
        string path = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString() + ".xlsx");
        using (var fs = new FileStream(path, FileMode.Create, FileAccess.ReadWrite))
        using (var spreadsheetDoc = SpreadsheetDocument.Create(fs, SpreadsheetDocumentType.Workbook, true))
        {
    ...
    public static string GeneratePptx()
    {
        string path = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString() + ".pptx");
        using (var fs = new FileStream(path, FileMode.Create, FileAccess.ReadWrite))
        using (var presentationDoc = PresentationDocument.Create(fs, PresentationDocumentType.Presentation, true))
        {
    ...
    {
        string path = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString() + ".rtf");
